...
> **Fork (yu1745) — v9.2.1-yu1745.4.** Adds an opt-in `allowUnverifiedApply`
...
>
> **9.2.1-yu1745.4 fixes**: the home directory now falls back to `os.homedir()`
...
> invisible. All diagnostics (run-start config values, gate refusals) are
> appended to a log file — `<cacheDir>/smart-compact.log`, e.g.
> `~/.pi/agent/.cache/smart-compact/smart-compact.log` (override with env
> `SMART_COMPACT_LOG`); nothing is printed to stdout/stderr. Also fixes the
